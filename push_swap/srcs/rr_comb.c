...
/*   Updated: 2024/05/16 17:55:18 by pvass            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_move *rr_rb(t_stack *temp, t_stack *b, int pos_a)
{
	...
	move->ra = 0;
	move->rr = pos_a;
	move->rb = ft_abs((pos_a - place_in_b(temp, &b, pos_a) % ft_stacksize(b)) % (ft_stacksize(b)));
...
t_move *rr_rrb(t_stack *temp, t_stack *b, int pos_a)
{
	...
	move->ra = 0;
	move->rr = pos_a;
	move->rrb = ft_abs((place_in_b(temp, &b, pos_a) % ft_stacksize(b) - pos_a) % ft_stacksize(b));
