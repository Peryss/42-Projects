...
/*   Updated: 2024/06/12 16:51:17 by pvass            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fractal	*fract_init(t_fractal *fractal)
{
	...
	fractal->y = 0;
	fractal->zoom = 300;
	fractal->color = 8388608;
	fractal->max_iterations = 35;
	...
	if (fractal->image == NULL)
		return (mlx_destroy_display(fractal->mlx), free(fractal->mlx),
			mlx_destroy_window(fractal->mlx, fractal->window), NULL);
